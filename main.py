...
ADMIN_CONTACT = os.environ.get("ADMIN_CONTACT", "@WQ_XR")
DEVELOPER_USERNAME = os.environ.get("DEVELOPER_USERNAME", "WQ_XR").lstrip("@")
...
# صور البوت
# ============================================================
WELCOME_IMAGE     = "https://h.top4top.io/p_3833693k31.jpg"
MAINTENANCE_IMAGE = "https://k.top4top.io/p_3833sgfyp1.jpg"
BANNED_IMAGE      = "https://i.top4top.io/p_38339f9zx1.jpg"
...
QUOTES = [
    ("خخخخخخخخخخخخخخخخخ.", "عرص انت"),
    ("خخخخخخخخ أبداً، أبداً، أبداً، أبداً.", "خخخخ خخخخ"),
    ("يبدو الأمر مستحيلاً دائماً حتى يتحقق.", "خول متناك"),
...
def force_sub_block(chat_id: int, uid: int) -> bool:
    ...
    for ch in unsubscribed:
        ...
        if ch["ch_type"] == "public":
            ...
            kb.add(types.InlineKeyboardButton(f"👁️‍🗨️ اشترك ← {label}", url=join_url, style="primary"))
        else:
            invite_link = (ch.get("invite_link") or "").strip()
            if invite_link:
                kb.add(types.InlineKeyboardButton(f"👁️‍🗨️ اشترك ← {label}", url=invite_link, style="primary"))
            else:
...
def main_menu_markup(is_admin: bool = False) -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        types.InlineKeyboardButton(" رفع ملف جديد", callback_data="proj_new",
    ...
    kb.add(
        types.InlineKeyboardButton(" 𝙁𝙇𝙄𝙓 𝙃𝑶𝙎𝙏", callback_data="noop_sig",
...
def show_main_menu(chat_id: int, uid: int, edit_message_id: Optional[int] = None) -> None:
    ...
    display_name = user.get("first_name") or "فليكس"
...
def main_terminal_callback(call):
    ...
    bot.send_message(call.message.chat.id, "🌐 استخدم التيرمنال من قائمة بوتاتك ← اختر بوت ← زر التيرمنال.")
...
def start_new_project(chat_id: int, uid: int) -> None:
    ...
    bot.send_message(
        ...
        "📛 ارسل اسم لي استضافه ملفك (ارقام-حروف).\n"
        "أو /cancel للإلغاء.",
        reply_markup=back_main_kb(),
    )
